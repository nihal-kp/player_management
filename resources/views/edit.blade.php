...
<div class="container col-9 col-md-7 col-lg-7 center">
    <h3 class="my-4">Update Details</h3>
    <form class="form-horizontal" action="/update/{{$player->id}}" method="POST">
        @csrf
        @method("PUT")
        <div class="form-group">
            <label for="name">Name:</label>
            <input type="text" class="form-control" placeholder="Enter name of the player" name="name" value="{{ $player->name }}" required>
            @error("name")
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" placeholder="Enter high score" name="score" value="{{ $player->score }}" required>
            @error("score")
